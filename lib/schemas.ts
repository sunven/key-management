...
export const tagSchema = z
  ...
  .regex(/^[\w\u4e00-\u9fa5-]+$/, 'Tag can only contain letters, numbers, Chinese characters, hyphens, and underscores');
...
export const groupItemSchema = z.object({
  ...
  value: z.string().min(1, 'Value is required').max(5000, 'Value must be 5000 characters or less'),
  tags: z.array(tagSchema).max(20, 'Maximum 20 tags allowed').optional(),
});

export const groupItemUpdateSchema = z.object({
  key: z.string().min(1, 'Key is required').max(100, 'Key must be 100 characters or less').optional(),
  value: z.string().min(1, 'Value is required').max(5000, 'Value must be 5000 characters or less').optional(),
  tags: z.array(tagSchema).max(20, 'Maximum 20 tags allowed').optional(),
});
