...
**验证：**
- [ ] `package.json` 包含 `resend` 和 `nanoid` 依赖
- [ ] `.env.local` 包含邮件配置（不提交到 git）
- [ ] `.env.example` 包含配置说明
...
**验证：**
- [ ] `prisma/schema.prisma` 包含新模型和枚举
- [ ] 迁移文件已创建（`prisma/migrations/`）
- [ ] 数据库表已创建（使用 `pnpm db:studio` 验证）
- [ ] Prisma Client 已更新（TypeScript 类型可用）
...
**验证：**
- [ ] `lib/share-utils.ts` 包含所有工具函数
- [ ] `lib/email.ts` 包含邮件发送逻辑
- [ ] 邮件模板渲染正确（手动测试）
- [ ] 加密/解密 token 正常工作
...
- [ ] PUBLIC 分享无需登录即可访问
- [ ] PRIVATE 分享需要登录且被邀请
- [ ] 邀请状态为 PENDING 时返回 403
- [ ] 邀请状态为 REJECTED 时返回 403
- [ ] 未被邀请用户返回 403
- [ ] 返回完整的 group 和 items 数据
...
- [ ] 拒绝邀请更新状态为 REJECTED
- [ ] 邮箱不匹配时返回错误
- [ ] 已响应的邀请可以重新接受（从 REJECTED 到 ACCEPTED）
- [ ] Token 验证正常工作
...
**验证：**
- [ ] 邮件在主流邮件客户端正常显示（Gmail、Outlook、Apple Mail）
- [ ] 移动端显示正常
- [ ] 按钮可点击
- [ ] 样式美观
