...
该规范定义了 Group 管理功能，允许用户创建和管理包含多个 key-value 配置项的 groups。每个 group 有名称和描述，group 内的每个 key-value 对象可以添加多个自定义标签，支持基于标签的过滤和搜索。该功能独立于现有的 Provider-Token 系统，主要用于管理 AI API 提供商的多个 URLs 和 tokens 配置。
